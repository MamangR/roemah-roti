      ...
      {sidebarOpen && (
        <div onClick={() => setSidebarOpen(false)} style={{ position: 'fixed', inset: 0, zIndex: 80, background: 'rgba(26,19,15,0.55)', backdropFilter: 'blur(2px)' }}>
          <div onClick={e => e.stopPropagation()} style={{ position: 'absolute', top: 0, left: 0, bottom: 0, width: '260px', background: 'var(--espresso)', display: 'flex', flexDirection: 'column', padding: '26px 18px', boxSizing: 'border-box', boxShadow: '4px 0 40px rgba(0,0,0,0.4)' }}>
            <div onClick={() => router.push('/admin')} style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '11px', padding: '0 8px 26px', borderBottom: '1px solid var(--text-on-hero-hair)' }}>
              ...
              <div><div style={{ fontSize: '10px', fontWeight: 600, letterSpacing: '.22em', color: 'var(--text-on-hero-dim)' }}>ROEMAH ROTI</div><div style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-on-hero)', marginTop: '2px' }}>Dashboard</div></div>
            </div>
            ...
            <div style={{ marginTop: '18px', paddingTop: '18px', borderTop: '1px solid var(--text-on-hero-hair)', display: 'flex', flexDirection: 'column', gap: '3px' }}>
              <div style={{ fontSize: '10px', fontWeight: 600, letterSpacing: '.16em', color: 'var(--text-on-hero-faint)', textTransform: 'uppercase', padding: '0 12px 8px' }}>Modul Lainnya</div>
              {[
                { label: 'Dashboard Overview', path: '/admin' },
                { label: 'Member Management', path: '/admin/members' },
                { label: 'Updates Management', path: '/admin/updates' },
                { label: 'Referral Management', path: '/admin/referrals' },
                { label: 'Settings', path: '/admin/settings' }
              ].map((mod, i) => (
                <div key={i} onClick={() => router.push(mod.path)} style={{ display: 'flex', alignItems: 'center', gap: '11px', padding: '9px 12px', borderRadius: '12px', color: 'var(--text-on-hero-dim)', cursor: 'pointer' }}>
                  <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: 'currentColor', opacity: 0.5, flex: 'none' }}></span>
      ...
      <div style={{ width: '250px', flex: 'none', background: 'var(--espresso)', display: 'flex', flexDirection: 'column', padding: '26px 18px', boxSizing: 'border-box' }} className="hidden md:flex">
        <div onClick={() => router.push('/admin')} style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '11px', padding: '0 8px 26px', borderBottom: '1px solid var(--text-on-hero-hair)' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-on-hero)', marginTop: '2px' }}>Dashboard</div>
          </div>
        </div>
            ...
            <div style={{ padding: '52px 40px 60px', animation: 'rmFade .3s cubic-bezier(.22,1,.36,1)' }}>
              ...
              <div style={{ marginTop: '22px', background: 'var(--surface-card)', border: '1px solid var(--border-card)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-card)', overflow: 'hidden' }}>
                ...
                {filteredRewards.map(r => {
                  ...
                  return (
                    <div key={r.id} className="rm-row" onClick={(e) => { e.stopPropagation(); setFormMode('edit'); setEditingId(r.id); setDraft({ ...r, visitsRequired: String(r.visitsRequired), validityDays: String(r.validityDays || 30), menuItemId: r.menuItemId || '' }); setScreen('form'); }} style={{ display: 'grid', gridTemplateColumns: '1.8fr 1fr .9fr 1fr', gap: '10px', padding: '15px 20px', borderTop: '1px solid var(--border-divider)', alignItems: 'center', fontSize: '13.5px', color: 'var(--text-body)' }}>
