      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          {subTab === 'newMenu' && newMenu.map((item) => (
            <div key={item.id} className="rp-info-card" style={{ display: 'flex', gap: '14px', alignItems: 'flex-start', padding: '16px' }}>
              {item.imageUrl && (
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                  <div style={{ fontSize: '14px', fontWeight: 600, color: '#3B2A22' }}>{item.name}</div>
                  <span style={{ fontSize: '9px', fontWeight: 600, letterSpacing: '0.06em', color: '#5C7B5A', background: 'rgba(122,150,116,0.14)', padding: '2px 6px', borderRadius: '999px' }}>NEW</span>
                </div>
          ...
          {subTab === 'promo' && promos.map((promo) => (
            <div key={promo.id} className="rp-info-card" style={{ padding: '16px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                <div style={{ fontSize: '14px', fontWeight: 600, color: '#3B2A22' }}>{promo.name}</div>
                <span style={{ fontSize: '10px', fontWeight: 600, letterSpacing: '0.02em', background: promo.promoStatus === 'Aktif' ? 'rgba(122,150,116,0.14)' : '#F5EFE6', color: promo.promoStatus === 'Aktif' ? '#5C7B5A' : '#A08A7B', padding: '3px 8px', borderRadius: '999px' }}>
